...
export function ProfileAccountInfo() {
  ...
  const t = useTranslations("ProfileAccountSection")
  ...
  const rows: Array<{
    ...
    value: string
    mono?: boolean
    badge?: "green" | "yellow" | "muted"
  }> = [
    ...(email ? [{ label: t("email"), value: email, mono: true }] : []),
    { label: t("emailStatus"), value: emailVerified ? t("verified") : t("unverified"), badge: (emailVerified ? "green" : "yellow") as "green" | "yellow" },
    ...(user.username ? [{ label: t("username"), value: user.username, mono: true }] : []),
    { label: t("memberSince"), value: createdAt },
    { label: t("lastSignIn"), value: lastSignIn },
    { label: t("twoFactor"), value: user.twoFactorEnabled ? t("enabled") : t("disabled"), badge: (user.twoFactorEnabled ? "green" : "muted") as "green" | "muted" },
  ]
            ...
            {row.badge ? (
              <Badge
                ...
                className={cn(
                  ...
                  row.badge === "muted" && "border-border text-muted-foreground",
                )}
              >
                {row.value}
              </Badge>
