...
/**
 * Builds sitemap entries for all configured static pages and blog posts for each supported locale.
 *
 * Each entry contains the localized URL, a lastModified timestamp (uses the page's `lastModified` when provided, otherwise the current time), `changeFrequency`, `priority`, and an `alternates.languages` map of locale → localized URL.
 *
 * @returns A Sitemap array with one entry per locale for every static page and blog post, including their alternates and metadata
 */
